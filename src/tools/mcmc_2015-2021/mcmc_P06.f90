module mcmc
    ...
    subroutine run_mcmc(st)
        ...
        do iDAsimu = 1, nDAsimu
            ...
            write(*,*) iDAsimu, "/", nDAsimu,  J_show_old, J_show_new, upgraded, accept_rate,search_scale
            ! do ishow = 1, 20
    ...
    ! & J_last(3),"/", J_new(3),";", J_last(4),"/", J_new(4),";", J_last(5),"/", J_new(5),";",upgraded, accept_rate
            ...
            if (upgraded .gt. temp_upgraded) then
                ...
                if (iDAsimu > 20000) then
                    if (accept_rate < 0.15) search_scale = search_scale * 0.99
                    if (accept_rate > 0.15) search_scale = search_scale * 2!1.01
                    if (search_scale > 0.05) search_scale = 0.05 
                    ! if (search_scale < 0.001) search_scale = 0.001
                end if
    ...
    subroutine generate_newPar()
        ...
        if (do_cov2createNewPars) then
            ...
            do while(parflag .gt. 0)    ! create the new coefnorm
                ...
                mc_DApar%coefnorm = mc_DApar%coefnorm * search_scale
                parflag = 0
...
999             continue
    ...
    subroutine costFuncObs_old()
        ...
        if(vars4MCMC%ANPP_Tree_y%existOrNot)then
            call CalculateCost(vars4MCMC%ANPP_Tree_y%mdData(:,4), vars4MCMC%ANPP_Tree_y%obsData(:,4),&
                 vars4MCMC%ANPP_Tree_y%obsData(:,5), J_cost)
            J_new(2) = J_new(2) + J_cost*10!*2000 !!10
        endif
        ...
        if(vars4MCMC%ANPP_Shrub_y%existOrNot)then
            call CalculateCost(vars4MCMC%ANPP_Shrub_y%mdData(:,4), vars4MCMC%ANPP_Shrub_y%obsData(:,4),&
                 vars4MCMC%ANPP_Shrub_y%obsData(:,5), J_cost)
            J_new(5) = J_new(5) + J_cost*10!*100 ! test *10 to better constrain
        endif
        ...
        if(vars4MCMC%BNPP_y%existOrNot)then
            call CalculateCost(vars4MCMC%BNPP_y%mdData(:,4), vars4MCMC%BNPP_y%obsData(:,4)*1.3,&
                 vars4MCMC%BNPP_y%obsData(:,5), J_cost)
            J_new(6) = J_new(6) + J_cost*10!*2000
        endif
        ...
        if(vars4MCMC%NPP_sphag_y%existOrNot)then
            call CalculateCost(vars4MCMC%NPP_sphag_y%mdData(:,4), vars4MCMC%NPP_sphag_y%obsData(:,4),&
                 vars4MCMC%NPP_sphag_y%obsData(:,5), J_cost)
            J_new(8) = J_new(8) + J_cost*10!*10000
        endif  
        ...
        if(vars4MCMC%gpp_d%existOrNot)then
            ...
            ! call CalculateCost_a(vars4MCMC%gpp_d%mdData(:,4), vars4MCMC%gpp_d%obsData(:,4),&
            !      vars4MCMC%gpp_d%obsData(:,5), J_cost)
            ! J_new(9) = J_new(9) + J_cost*10!*5000
            J_new(9) = J_new(9)*100
        endif
        ...
        if(vars4MCMC%er_h%existOrNot)then
            call CalculateCost(vars4MCMC%er_h%mdData(:,4), vars4MCMC%er_h%obsData(:,4),&
                 vars4MCMC%er_h%obsData(:,5), J_cost)
            J_new(10) = J_new(10) + J_cost*1000!*50000
        endif
        ...
        if(vars4MCMC%nee_h%existOrNot)then
            call CalculateCost(vars4MCMC%nee_h%mdData(:,4), vars4MCMC%nee_h%obsData(:,4),&
                 vars4MCMC%nee_h%obsData(:,5), J_cost)
            J_new(11) = J_new(11) + J_cost*1000!*10000
        endif

        if(vars4MCMC%cSoil_y%existOrNot)then
            call CalculateCost(vars4MCMC%cSoil_y%mdData(:,4), vars4MCMC%cSoil_y%obsData(:,4),&
                 vars4MCMC%cSoil_y%obsData(:,5), J_cost)
            J_new(12) = J_new(12) + J_cost/50!*10
        endif
        ...
        if(vars4MCMC%ch4_h%existOrNot)then
            call CalculateCost(vars4MCMC%ch4_h%mdData(:,4), vars4MCMC%ch4_h%obsData(:,4),&
                 vars4MCMC%ch4_h%obsData(:,5), J_cost)
            J_new(13) = J_new(13) + J_cost*10000
        endif
...
! enddo
        ...
        if(vars4MCMC%lai_shrub_d%existOrNot)then
            ...
            J_new(17) = J_new(17) + 0!J_cost*100
        endif
        ! print*, "mod lai shrub: ", vars4MCMC%lai_shrub_d%mdData(:,4)
        ! print*, "obs lai shrub: ", vars4MCMC%lai_shrub_d%obsData(:,4)
        ...
        if(vars4MCMC%lai_tree_d%existOrNot)then
            ...
            J_new(18) = J_new(18) + J_cost*1000!*1000
        endif
        ! print*, "mod lai tree: ", vars4MCMC%lai_tree_d%mdData(:,4)
        ! print*, "obs lai tree: ", vars4MCMC%lai_tree_d%obsData(:,4)

        ! photo_tree_h       
        if(vars4MCMC%photo_tree_h%existOrNot)then
            call CalculateCost(vars4MCMC%photo_tree_h%mdData(:,4), vars4MCMC%photo_tree_h%obsData(:,4)*1.2,&
                 vars4MCMC%photo_tree_h%obsData(:,5), J_cost)
            J_new(19) = J_new(19) + J_cost*100!*10000 ! 1000 a little lower 
        endif
        ! print*, "mod photo tree: ", vars4MCMC%photo_tree_h%mdData(:,4)
        ! print*, "obs photo tree: ", vars4MCMC%photo_tree_h%obsData(:,4)
        ...
        ! if(AMIN1(1.0, exp(-sum(delta_J(1:2)))) .gt. cs_rand)then
            ...
            !         if(AMIN1(1.0, exp(-sum(delta_J(7:8)))) .gt. cs_rand)then
                        ...
                        !     if(AMIN1(1.0, exp(-delta_J(10))) .gt. cs_rand)then
                            ! if(AMIN1(1.0, exp(-(J_new(9) - J_last(9)))) .gt. cs_rand)then
            upgraded = upgraded + 1
            J_last = J_new
                            ! endif
                        ! endif
    ...
    subroutine CalculateCost_a(datMod4MCMC, datObs4MCMC, stdObs4MCMC, JCost)
        ...
        real(8) JCost, sum_xy, sum_xx, div_xy_xx
        ...
        do iLine = 1, nLine
            if(datObs4MCMC(iLine) .gt. -999 .and. datMod4MCMC(iLine) .gt. -999)then
                sum_xy  = sum_xy + datObs4MCMC(iLine)*datMod4MCMC(iLine)
                sum_xx  = sum_xx + datObs4MCMC(iLine)*datObs4MCMC(iLine)
            endif
        enddo
        if(sum_xx > 0) div_xy_xx = sum_xy/sum_xx
        div_xy_xx = abs(div_xy_xx-1)*100
        JCost = div_xy_xx
        return
    end subroutine CalculateCost_a
...
subroutine CalculateCost(datMod4MCMC, datObs4MCMC, stdObs4MCMC, JCost)
        ...
        real(8), intent(in) :: datMod4MCMC(:), datObs4MCMC(:), stdObs4MCMC(:)
        integer nLine, iLine, nCost
        real(8) JCost, dObsSimu, std4cal, JCost_std, mean_Val, std_val, var_val
        ...
        ! test for entile observation dataset
        mean_Val = sum(datObs4MCMC)/size(datObs4MCMC)
        var_val  = sum((datObs4MCMC - mean_Val)**2) / real(size(datObs4MCMC))
        ...
        do iLine = 1, nLine
            if(datObs4MCMC(iLine) .gt. -999 .and. datMod4MCMC(iLine) .gt. -999)then
                ...
                ! ! std4cal  = abs(0.2*datObs4MCMC(iLine))
                ! if (std4cal <=0) std4cal = 1
                std4cal  = datObs4MCMC(iLine)*0.3
                JCost    = JCost + (dObsSimu*dObsSimu)!/(std4cal)!*std4cal)
                JCost_std = JCost_std + (datObs4MCMC(iLine) - mean_Val) * &
                                         (datObs4MCMC(iLine) - mean_Val)
            endif
        enddo
        ! if (JCost_std > 0) then
        !     Jcost = JCost/var_val
        ! else
        !     Jcost = JCost/(0.3*datObs4MCMC(1)*0.3*datObs4MCMC(1))
        ! endif
        ! JCost = JCost
        if(nCost .gt. 0) JCost=sqrt(JCost/nCost)*10!JCost=JCost*100/nCost! * 1000
        return ! JCost

...
!             print*, "sla: ", in_params_vals%sp_params(1)%SLA, in_params_vals%sp_params(2)%SLA, in_params_vals%sp_params(3)%SLA 
! print*, "vcmax0: ", in_params_vals%sp_params(1)%Vcmax0, in_params_vals%sp_params(2)%Vcmax0, in_params_vals%sp_params(3)%Vcmax0
...
999             continue
    ...
    subroutine costFuncObs_old()
        ...
        if(vars4MCMC%cPlant_tree_y%existOrNot)then
            call CalculateCost(vars4MCMC%cPlant_tree_y%mdData(:,4), vars4MCMC%cPlant_tree_y%obsData(:,4),&
                 vars4MCMC%cPlant_tree_y%obsData(:,5), J_cost)
            J_new(1) = J_new(1) + 0!J_cost*10000
        endif
        ! print*, "mod_tree_biomass: ", vars4MCMC%cPlant_tree_y%mdData(:,4)
        ! print*, "obs_tree_biomass: ", vars4MCMC%cPlant_tree_y%obsData(:,4)-300

        ! ! ANPP_Tree_y
        if(vars4MCMC%ANPP_Tree_y%existOrNot)then
            call CalculateCost(vars4MCMC%ANPP_Tree_y%mdData(:,4), vars4MCMC%ANPP_Tree_y%obsData(:,4),&
                 vars4MCMC%ANPP_Tree_y%obsData(:,5), J_cost)
            J_new(2) = J_new(2) + J_cost!*100000
        endif
...
! enddo
        ...
        if(vars4MCMC%lai_tree_d%existOrNot)then
            ...
            J_new(18) = J_new(18) + J_cost*1000!*10000
        endif
        ! print*, "lai_tree_mod: ",vars4MCMC%lai_tree_d%mdData(:,4)
        ! print*, "lai_tree_obs: ", vars4MCMC%lai_tree_d%obsData(:,4)
        ...
        if(vars4MCMC%photo_tree_h%existOrNot)then
            ...
            J_new(19) = J_new(19) + J_cost*100!*1000
        endif
